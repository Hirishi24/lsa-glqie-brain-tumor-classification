...
def _train_eval_method(
    ...
    global_cols = [f"global_dct_{i}" for i in range(12)]
    local_cols = [f"local_dct_{i}" for i in range(12)]
    ...
    if method == "lesion_size_only":
        ...
        coeff = build_coefficients(g, l, "proposed_lsa_glqie", shuffle_categories(categories, seed), seed)
    else:
        coeff = build_coefficients(g, l, "fixed_4g_4l" if method == "svm_classical_proposed" else method, categories, seed)
    ...
    is_quantum = method not in {"classical_proposed", "lesion_size_only", "svm_classical_proposed"}
    if is_quantum:
        batch_size = int(cfg.get("runtime", {}).get("batch_size", 256))
        x_train_model = quantum_features(x_train, device, batch_size, int(cfg["quantum"].get("shots", 0)), float(cfg["quantum"].get("noise_level", 0.0)), seed, rounds=int(cfg["quantum"]["reuploading_rounds"]))
        x_val_model = quantum_features(x_val, device, batch_size, int(cfg["quantum"].get("shots", 0)), float(cfg["quantum"].get("noise_level", 0.0)), seed + 1, rounds=int(cfg["quantum"]["reuploading_rounds"]))
        x_test_model = quantum_features(x_test, device, batch_size, int(cfg["quantum"].get("shots", 0)), float(cfg["quantum"].get("noise_level", 0.0)), seed + 2, rounds=int(cfg["quantum"]["reuploading_rounds"]))
    else:
        x_train_model, x_val_model, x_test_model = x_train, x_val, x_test
    if method == "svm_classical_proposed":
